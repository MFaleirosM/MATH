...
Original file is located at
    https://colab.research.google.com/drive/10pCWFoTrsLF-7bpVrQ8JviqLuENZoQ9Q
"""
...
def convert_latex_delimiters(text: str) -> Tuple[str, int]:
    ...
    def convert_and_count(pattern, replacement):
    ...
    text = convert_and_count(r'\\begin\{equation\*\}(.*?)\\end\{equation\*\}', r'$$\1$$')

    # \begin{equation}...\end{equation} -> $$...$$
    text = convert_and_count(r'\\begin\{equation\}(.*?)\\end\{equation\}', r'$$\1$$')

    # \begin{align*}...\end{align*} -> $$...$$
    text = convert_and_count(r'\\begin\{align\*\}(.*?)\\end\{align\*\}', r'$$\1$$')

    # \begin{align}...\end{align} -> $$...$$
    text = convert_and_count(r'\\begin\{align\}(.*?)\\end\{align\}', r'$$\1$$')

    # \begin{gather*}...\end{gather*} -> $$...$$
    text = convert_and_count(r'\\begin\{gather\*\}(.*?)\\end\{gather\*\}', r'$$\1$$')
    ...
    # Special cases with potential false positives - we'll be more careful
    # Need to handle cases where $ might appear in regular text
